# filepath: c:\public_work\app_project\streamlit_app.py
import streamlit as st
import matplo            def extract_path_states(steps, a_cap, b_cap):
                states = [(0, 0)]  # 初期状態
                for step in steps:
...
if is_solvable(a, b, goal):
    ...
    show_steps = st.button(""操作手順を表示"")
    show_graph = st.button(""グラフで表示"")
    
    if show_steps or show_graph:
        ...
        def simulate_pour_path(path, a_cap, b_cap):
            ...
            for i in range(1, len(path)):
                ...
                if a2 == a_cap and a1 != a_cap:
                    ...
                    t = b2 - b1
                    log.append(f""ABに{t}L注ぐ  ({a2}L, {b2}L)"")
                elif b2 < b1 and a2 > a1:
                    t = a2 - a1
                    log.append(f""BAに{t}L注ぐ  ({a2}L, {b2}L)"")
                else:
        ...
        if show_graph:
            ...
            for i, (a_val, b_val) in enumerate(states):
                ...
                if i == 0:
                    ...
                    ax.text(-a-0.5, y_pos, f""{i} {step_description}"", 
                            ha='right', va='center')
            ...
            # タイトル
            ax.set_title(f""B容器を基準に{goal}Lを測る手順 （ステップ別一覧左右分離）"")
